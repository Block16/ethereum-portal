...
var LedgerEth = function(comm) {
  ...
  this.getAddress = function(path, callback, boolDisplay, boolChaincode) {
  	...
  	var buffer = new window.buffer.Buffer(5 + 1 + splitPath.length * 4);
  	buffer[0] = 0xe0;
  	buffer[1] = 0x02;
  	...
  	var localCallback = function(response, error) {
  		if (typeof error != "undefined") {
  			...
  			var result = {};
  			response = new window.buffer.Buffer(response, 'hex');
  ...
  this.signTransaction = function(path, rawTxHex, callback) {
  	var splitPath = LedgerEth.splitPath(path);
  	var offset = 0;
  	var rawTx = new window.buffer.Buffer(rawTxHex, 'hex');
  	var apdus = [];
  	while (offset != rawTx.length) {
  		...
  		var buffer = new window.buffer.Buffer(offset == 0 ? 5 + 1 + splitPath.length * 4 + chunkSize : 5 + chunkSize);
  		buffer[0] = 0xe0;
  		buffer[1] = 0x04;
  ...
  this.getAppConfiguration = function(callback) {
  	var buffer = new window.buffer.Buffer(5);
  	buffer[0] = 0xe0;
  	buffer[1] = 0x06;
  	buffer[2] = 0x00;
  	buffer[3] = 0x00;
  ...
  this.signPersonalMessage_async = function(path, messageHex, callback) {
    var splitPath = LedgerEth.splitPath(path);
    var offset = 0;
    var message = new window.buffer.Buffer(messageHex, 'hex');
    var apdus = [];
    var response = [];
    var self = this;
    while (offset != message.length) {
      ...
      var buffer = new window.buffer.Buffer(offset == 0 ? 5 + 1 + splitPath.length * 4 + 4 + chunkSize : 5 + chunkSize);
      buffer[0] = 0xe0;
      buffer[1] = 0x08;
      buffer[2] = (offset == 0 ? 0x00 : 0x80);
      buffer[3] = 0x00;
